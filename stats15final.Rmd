...
aac <-
  aac  %>%
  filter(!(Consecutive)) %>%
  select(-c(Consecutive, Num.Action))
```
...
It may be interesting to see if certain breeds spend more time in the shelter than others before being adopted.
...
We still see a right skew, though not as intensely. There are some that stick out, so let's look at the ones at the top.
...
aac %>%
  ...
  filter(Action=="outcome", Type=="Adoption") %>%
  group_by(Breed) %>%
  summarise(avg_time_before_adoption=mean(Time.From.Last.Action)) %>%
  arrange(desc(avg_time_per_intake)) %>%
  head(10)
```

While the "Dogo Argentino" may seem random, its appearance actually closely resembles that of a Pit Bull.
