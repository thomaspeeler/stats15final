...
First, let's see how each breed compares to each other for their mean time before adoption.
...
```{r}
#im working on this dont mind me
```
...
```{r}
breed_v_time <- lm(Days.From.Last.Action~Breed, data=aac_m)
...
0.06 is quite a small R-squared value, but let's see how it stacks up to our other variables.

## Age Coefficients

```{r}
age_v_time <- lm(Days.From.Last.Action~Age, data=aac_m)
summary(age_v_time)
...
While we have a very small p-value, indicating a relationship between our independent and response variables, our R-squared value is also quite low, reflecting quite a bit of variability in adoption times when comparing them to ages.

## Color

```{r}
#um i need james part for this so wip?
```
...
Despite very low p-values, our overall R-squared is only 0.159, which, while higher than for the variables on their own, is still very low. However, the p-values still tell us that there is a relationship between our variables.
...
tmp %>%
  mutate(pred=predict(full_model, tmp))
```
