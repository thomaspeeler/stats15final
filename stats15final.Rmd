...
[intro here]
Motivating Question: What attributes affect whether a dog gets adopted?
...
To help with this part of the cleaning, we'll be adding a column that will specify how long each intake/outcome lasted (in seconds) until the next one for each animal. For the first intake/outcome for an animal, an NA will be in the column.
...
```{r}
#11/21 NOTE: I DONT KNOW IF THIS IS THE RIGHT THING TO DO; NEED TO ASK MILES ABT IT
aac_r <-
  aac_f  %>%
  filter(!(Consecutive)) %>%
...
#maybe we just do the same thing we did for the breed column and just do the main color? i was hesitant of that bc the most common color is "black/white," and lumping in the most common with other colors seemed off idk

#the way that we have it right now causes a massive spike in the density plot around 1600 occurances which seems like a problem so theres a very high chance this part will change a lot tbh

#honestly the solution might just be to not change it at all ugh
...
#look i know this is slow but like whatever right??? it works idc im so tired why isnt it winter break yet
...
```{r}
#note: not to bring up problems without fixing them buttttttttttttt there are some dogs that have been in and out of the shelter like >40 times, and when analysing for adoption rates i feel like entries like that really muddy the waters. maybe its all relative and like we are only comparing dogs in the table against each other so it doesnt matter too much but idk. it might be a good idea to look at each animal ID separately and make a column for if each animal was every adopted even once and use that for analysis when looking at adoption vs breed or color or age. idk this is a problem for future me

#IDEAS FOR EXPLORATORY ANALYSIS GRAPHS TO DO:
#bar graphs with explanatory var on x (breed, color, age, etc) and stacked bars with adoption and non-adoption outcomes or adopted and non-adopted dogs
#look at distribution of age and time between actions
#more ideas here
```
